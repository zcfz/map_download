...
function mainnAllXY(bounding, Minlevel, Maxlevel) {
    for (let i = Minlevel; i <= Maxlevel; i++) {
        ...
        //     for(t = 0; t < Math.pow(2,i); t ++) {
        //         all.push({x: j, y: t})
        //     }
        // }
        // console.log(i)
        alli.t = i // 层级
        alli.x = [0, Math.pow(2,i)] // 瓦片横坐标范围（左至右）
        alli.y = [0, Math.pow(2,i - 1)] // 瓦片纵坐标范围（下至上）
        // let p1 = calcXY(bounding[2], bounding[3], i);
        // let p2 = calcXY(bounding[0], bounding[1], i);
        // console.log(alli, all, 'all')
        all.push(alli)
...
function task() {
    for (let z = 0; z <= all.length - 1; z++) {
        let  yMax = z == 0 ? all[z].y[1] : (all[z].y[1] + Math.pow(2, z))
        for (let x = all[z].x[0]; x <= all[z].x[1]; x++) {
            for (let y = all[z].y[0]; y <= yMax; y++) {
                // 将下载任务推入队列
                ++sum
